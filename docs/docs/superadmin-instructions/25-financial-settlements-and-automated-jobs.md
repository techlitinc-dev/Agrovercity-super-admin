# Superadmin Implementation Guide — Module 25: Financial Settlements & Automated Cron Jobs

> **Document ID:** `SOP-25`  
> **Module Scope:** `Financial Settlements & Automated Cron Jobs`  
> **Target Collections:** `settlements`, `transporter_payouts`, `seller_payouts`, `cron_job_logs`  
...
|---|---|---|---|
| `settlements` | Primary document collection for Financial Settlements & Automated Cron Jobs | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `transporter_payouts` | Primary document collection for Financial Settlements & Automated Cron Jobs | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `seller_payouts` | Primary document collection for Financial Settlements & Automated Cron Jobs | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `cron_job_logs` | Primary document collection for Financial Settlements & Automated Cron Jobs | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
...
- **Review**: Review pending transporter freight and seller produce payouts in the daily settlement batch
- **Manually**: Manually trigger on-demand settlement calculation run
- **Mark**: Mark settlements as disbursed with bank UTR / NEFT reference numbers
- **Place**: Place suspicious transactions or dispute-locked bookings on legal settlement hold
- **Audit**: Audit platform service fee deductions and GST invoice generation
...
```
+----------------------------------------------------------------------------------------------------+
| AGROVERCITY SUPERADMIN  ::  FINANCIAL SETTLEMENTS & AUTOMATED CRON JOBS                          [ Admin User: root@agrovercity ] |
+----------------------------------------------------------------------------------------------------+

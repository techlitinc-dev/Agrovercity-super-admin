# Superadmin Implementation Guide — Module 08: Transport Logistics & Fleet Operations

> **Document ID:** `SOP-08`  
> **Module Scope:** `Transport Logistics & Fleet Operations`  
> **Target Collections:** `vehicles`, `transport_bookings`, `transporter_settlements`  
> **Superadmin UI Path:** `/admin/transport`  
...
|---|---|---|---|
| `vehicles` | Primary document collection for Transport Logistics & Fleet Operations | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `transport_bookings` | Primary document collection for Transport Logistics & Fleet Operations | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `transporter_settlements` | Primary document collection for Transport Logistics & Fleet Operations | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
...
- **Verify**: Verify transporter fleet registration (RC book, commercial insurance, fitness certificate)
- **Live**: Live dispatch map showing all active booked vehicles and routes
- **Arbitrate**: Arbitrate booking disputes, no-shows, and breakdown incidents
- **Manage**: Manage base fares and per-km pricing bands by district and vehicle class
- **Audit**: Audit proof-of-delivery (POD) documentation before releasing payout
...
```
+----------------------------------------------------------------------------------------------------+
| AGROVERCITY SUPERADMIN  ::  TRANSPORT LOGISTICS & FLEET OPERATIONS                          [ Admin User: root@agrovercity ] |
+----------------------------------------------------------------------------------------------------+
...
|---|---|---|---|---|---|
| `GET` | `List /v1/admin/transport/vehicles` | List registered fleet with KYC doc status | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `POST` | `Approve /v1/admin/transport/vehicles/{id}/verify` | Approve vehicle commercial papers | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `GET` | `List /v1/admin/transport/bookings` | List active and historical bookings | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `PUT` | `Manual /v1/admin/transport/bookings/{id}/status` | Manual intervention in trip status | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |

# Superadmin Implementation Guide — Module 11: AI Advisory, Disease Scan & Pest Radar

> **Document ID:** `SOP-11`  
> **Module Scope:** `AI Advisory, Disease Scan & Pest Radar`  
> **Target Collections:** `advisory_scans`, `pest_alerts`, `soil_tests`, `crop_cycles`  
> **Superadmin UI Path:** `/admin/advisory`  
...
|---|---|---|---|
| `advisory_scans` | Primary document collection for AI Advisory, Disease Scan & Pest Radar | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `pest_alerts` | Primary document collection for AI Advisory, Disease Scan & Pest Radar | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `soil_tests` | Primary document collection for AI Advisory, Disease Scan & Pest Radar | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `crop_cycles` | Primary document collection for AI Advisory, Disease Scan & Pest Radar | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
...
- **Monitor**: Monitor AI disease diagnosis accuracy and false positive feedback
- **Publish**: Publish district-wide pest and disease outbreak radar alerts with geofencing
- **Upload**: Upload laboratory soil test result PDFs and trigger farmer notifications
- **Configure**: Configure NPK recommendation algorithms based on ICAR guidelines
- **Audit**: Audit market saturation calculation parameters and crop substitution models
...
```
+----------------------------------------------------------------------------------------------------+
| AGROVERCITY SUPERADMIN  ::  AI ADVISORY, DISEASE SCAN & PEST RADAR                          [ Admin User: root@agrovercity ] |
+----------------------------------------------------------------------------------------------------+
...
|---|---|---|---|---|---|
| `GET` | `Audit /v1/admin/advisory/scans` | Audit disease scans and confidence metrics | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `POST` | `Broadcast /v1/admin/advisory/pest-alerts` | Broadcast geofenced pest outbreak alert | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `GET` | `List /v1/admin/soil-tests` | List pending soil sample tests | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `POST` | `Upload /v1/admin/soil-tests/{id}/results` | Upload lab result PDF and notify farmer | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |

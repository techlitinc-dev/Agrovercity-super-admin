# Superadmin Implementation Guide — Module 25: Financial Settlements, Commissions & Scheduled Jobs

> **Document ID:** `SOP-25`  
> **Module Scope:** `Financial Settlements, Commissions & Scheduled Jobs`  
> **Target Collections:** `settlements`, `platform_config/settlements`, `cron_job_logs`  
> **Superadmin UI Path:** `/admin/settlements`  
...
Automated financial reconciliation engine: nightly batch aggregation of completed transport trips, equipment rentals, and broker deals; platform commission deduction (10%, 12%, 2%), and scheduled rent reminder triggers.
...
|---|---|---|---|
| `settlements` | Primary document collection for Financial Settlements, Commissions & Scheduled Jobs | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `platform_config/settlements` | Primary document collection for Financial Settlements, Commissions & Scheduled Jobs | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `cron_job_logs` | Primary document collection for Financial Settlements, Commissions & Scheduled Jobs | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
...
- **Central**: Central payout management dashboard: review weekly settlement aggregation rows
- **Approve**: Approve net payout amounts and initiate bulk bank transfers via RazorpayX / NEFT
- **Configure**: Configure platform commission percentages (Transporter: 10%, Equipment: 12%, Broker: 2%)
- **Trigger**: Trigger manual settlement batch re-computation with custom start/end periods
- **Monitor**: Monitor Cloud Scheduler cron execution status, error logs, and execution times
...
```
+----------------------------------------------------------------------------------------------------+
| AGROVERCITY SUPERADMIN  ::  FINANCIAL SETTLEMENTS, COMMISSIONS & SCHEDULED JOBS                          [ Admin User: root@agrovercity ] |
+----------------------------------------------------------------------------------------------------+
...
|---|---|---|---|---|---|
| `GET` | `List /v1/admin/settlements` | List settlements with status filters (pending/approved/paid) | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `POST` | `Mark /v1/admin/settlements/{id}/mark-paid` | Mark settlement disbursed with payment ref | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `POST` | `Trigger /v1/admin/jobs/settlements/run` | Trigger manual settlement batch run | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `PUT` | `Update /v1/admin/platform-config/commissions` | Update commission rates | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |

# Superadmin Implementation Guide — Module 16: Land Records Registry (7/12 & 8A Utara)

> **Document ID:** `SOP-16`  
> **Module Scope:** `Land Records Registry (7/12 & 8A Utara)`  
> **Target Collections:** `land_records_712`, `users/{uid}/imported_records`  
> **Superadmin UI Path:** `/admin/land-records`  
...
|---|---|---|---|
| `land_records_712` | Primary document collection for Land Records Registry (7/12 & 8A Utara) | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
| `users/{uid}/imported_records` | Primary document collection for Land Records Registry (7/12 & 8A Utara) | `id`, `createdAt`, `updatedAt`, `status`, `userId` | Audit logged, soft-delete enabled |
...
- **Monitor**: Monitor State revenue portal integration gateway availability and latency
- **Audit**: Audit land record search queries and cache hit/miss performance
- **Resolve**: Resolve land record parsing errors for unusual script or survey notations
- **Manually**: Manually provision verified land records during state portal downtime
- **Export**: Export land ownership verification logs for government reporting
...
```
+----------------------------------------------------------------------------------------------------+
| AGROVERCITY SUPERADMIN  ::  LAND RECORDS REGISTRY (7/12 & 8A UTARA)                          [ Admin User: root@agrovercity ] |
+----------------------------------------------------------------------------------------------------+
...
|---|---|---|---|---|---|
| `GET` | `Revenue /v1/admin/land-records/status` | Revenue portal gateway health and latency | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `POST` | `Seed /v1/admin/land-records/seed` | Seed verified village 7/12 records to cache | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |
| `GET` | `Audit /v1/admin/land-records/audit-log` | Audit trail of user 7/12 imports | JSON payload | Enveloped object | `401`, `403`, `404`, `422` |

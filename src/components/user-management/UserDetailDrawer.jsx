      ...
      <div className="w-full max-w-2xl bg-slate-900 border-l border-slate-800 shadow-2xl flex flex-col h-full animate-in slide-in-from-right duration-200">
        ...
        <div className="px-6 py-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-full bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center font-bold text-emerald-400 text-sm">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {user.womenMode && (
                  <span className="inline-flex items-center gap-1 text-[11px] bg-pink-950 text-pink-300 border border-pink-700/50 px-2 py-0.2 rounded-full font-semibold">
                    <HeartHandshake className="w-3 h-3" /> Women SHG
                  </span>
                )}
              </div>
              <div className="text-xs text-slate-400 flex items-center gap-2 font-mono">
                ...
                <span>•</span>
                <span className="capitalize text-emerald-400 font-semibold">{user.status}</span>
              </div>
            </div>
          </div>
        ...
        {/* Tab Navigation */}
        <div className="px-6 bg-slate-950/60 border-b border-slate-800 flex items-center gap-4 text-xs font-medium shrink-0 overflow-x-auto">
          {[
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-emerald-950/40 via-slate-900 to-slate-900 border border-emerald-500/30 rounded-xl p-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-slate-400 mt-0.5">
                    Currently Active View: <strong className="text-slate-200">{user.activePersona || 'Farmer'}</strong>
                  </div>
                </div>
                <button
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-3 flex justify-between items-center">
                    <div>
                  ...
                  <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-3 flex justify-between items-center">
                    <div>
                      ...
                      <span className="text-slate-100 font-mono font-semibold">{user.mobile}</span>
                    </div>
                    <button
                  ...
                  <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-3 flex justify-between items-center">
                    <div>
                      ...
                      <span className="text-slate-100">{user.email || 'None Registered'}</span>
                    </div>
                  </div>

                  <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-3 flex justify-between items-center">
                    <div>
                      <span className="text-slate-500 block text-[11px]">Aadhaar Masked (DPDP Enforced)</span>
                      <span className="text-emerald-400 font-mono font-semibold">{user.aadhaarMasked || 'XXXX-XXXX-4812'}</span>
                    </div>
                    <span className="text-[10px] text-emerald-500 bg-emerald-950 px-1.5 py-0.5 rounded border border-emerald-500/30">
                      Masked
                    </span>
                  </div>

                  <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-3 sm:col-span-2 flex justify-between items-center">
                    <div>
                      <span className="text-slate-500 block text-[11px]">Geographic Jurisdiction</span>
                      <span className="text-slate-200">{user.geoCity || 'Kolhapur, Maharashtra'}</span>
                    </div>
              ...
              <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col justify-end">
                    ...
                    <label className="flex items-center gap-2.5 cursor-pointer bg-slate-900 border border-slate-700 rounded-lg px-3 py-2">
                      <input
                        type="checkbox"
                ...
                <div className="space-y-1.5 pt-2">
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-700/80 rounded-lg px-3 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  />
              ...
              <div className="space-y-3">
                {linkedPersonas.map(([pKey, pData]) => {
                  ...
                  return (
                    <div
                      key={pKey}
                      className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 flex flex-col gap-2.5"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                          <span className="font-bold text-slate-100 text-sm">{pKey}</span>
                          {isPrimary && (
                            <span className="inline-flex items-center gap-1 text-[11px] bg-amber-950 text-amber-300 border border-amber-600/40 px-2 py-0.5 rounded-full font-semibold">
                              <Star className="w-3 h-3 fill-amber-400 text-amber-400" /> Starred Primary
                            </span>
                          )}
                          {isActive && (
                            <span className="text-[11px] bg-emerald-950 text-emerald-300 border border-emerald-600/40 px-2 py-0.5 rounded-full font-semibold">
                              Active View
                            </span>
                          )}
                        </div>
            ...
            <div className="space-y-4">
              ...
              {user.farmPolygon ? (
                <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="font-bold text-white text-sm">{user.farmPolygon.title}</div>
                      <div className="text-xs text-slate-400 font-mono">{user.farmPolygon.surveyNumber}</div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-800">
                    <div>
                      ...
                      <span className="text-slate-300">{user.farmPolygon.village}, {user.farmPolygon.taluka}, {user.farmPolygon.district}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[11px]">GPS Center</span>
                      <span className="text-slate-300 font-mono">{user.farmPolygon.center?.join(', ')}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[11px]">Soil Classification</span>
                      <span className="text-slate-300">{user.farmPolygon.soilType}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[11px]">Irrigation Lift</span>
                      <span className="text-slate-300">{user.farmPolygon.irrigationSource}</span>
                    </div>
                  </div>

                  <div className="text-[11px] text-slate-500 pt-2 border-t border-slate-800 flex justify-between items-center">
                    <span>Vertices Mapped: {user.farmPolygon.coordinates?.length || 0} Points</span>
                    <span className="text-emerald-400">✓ Audit Sealed</span>
                  </div>
                </div>
              ) : (
                <div className="py-8 text-center bg-slate-950/60 border border-dashed border-slate-800 rounded-xl text-slate-400 text-xs">
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {bookings.map((b) => (
                        <tr key={b.id} className="hover:bg-slate-900/40">
                          <td className="py-2.5 px-3 font-mono text-slate-300 font-bold">{b.id}</td>
                          <td className="py-2.5 px-3">
                            <span className="px-1.5 py-0.5 rounded text-[10px] bg-slate-800 text-slate-300">
                              {b.persona}
                            </span>
                          </td>
                          <td className="py-2.5 px-3 text-slate-200">{b.service}</td>
                          <td className="py-2.5 px-3 text-slate-400">{b.counterparty}</td>
                          <td className="py-2.5 px-3 text-right font-mono font-semibold text-emerald-400">
                            ₹{b.amount.toLocaleString('en-IN')}
                          </td>
                          <td className="py-2.5 px-3">
                            <span className="capitalize text-[11px] font-semibold text-slate-300">
                              {b.status}
                            </span>
                          </td>
                        </tr>
          ...
          {activeTab === 'audit' && (
            <div className="space-y-3">
              ...
              {loadingAudit ? (
                ...
                <div className="py-8 text-center text-slate-500 text-xs">No audit logs for this user.</div>
              ) : (
                <div className="space-y-2.5">
                  {auditLogs.map((log) => (
                    <div
                      key={log.id}
                      className="bg-slate-950/80 border border-slate-800 rounded-lg p-3 text-xs space-y-1"
                    >
                      ...
                      <div className="text-[11px] text-slate-500 font-mono flex items-center justify-between pt-1">
                        <span>Prev: <span className="text-slate-400">{log.previousState}</span></span>
                        <span>Admin: <span className="text-slate-400">{log.adminUid}</span></span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'json' && (
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-400">Complete Firestore Enveloped User Document</span>
                <button

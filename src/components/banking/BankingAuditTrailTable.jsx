...
export default function BankingAuditTrailTable({
  auditLogs = [],
  loading = false,
  onRefresh
}) {
  ...
  const getActionBadge = (action) => {
    switch (action) {
      case 'PENNY_DROP_OVERRIDDEN':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
            <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Penny-Drop Overridden
          </span>
        )
      case 'PRIMARY_ACCOUNT_DESIGNATED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-sky-50 text-sky-700 border border-sky-200">
            <Building2 className="w-3 h-3 text-sky-600" /> Primary Account Designated
          </span>
        )
      case 'DPDP_MASKING_AUDITED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-teal-50 text-teal-700 border border-teal-200">
            <Lock className="w-3 h-3 text-teal-600" /> DPDP Masking Audited
          </span>
        )
      case 'LOAN_UNDERWRITTEN_APPROVED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-300">
            <ShieldCheck className="w-3 h-3 text-emerald-600" /> Loan Approved (Dual-Sign)
          </span>
        )
      case 'LOAN_REJECTED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-200">
            <AlertTriangle className="w-3 h-3 text-rose-600" /> Loan Rejected
          </span>
        )
      case 'LOAN_DISBURSED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200">
            <Landmark className="w-3 h-3 text-indigo-600" /> Facility Disbursed
          </span>
        )
      case 'KCC_LIMIT_ADJUSTED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-purple-50 text-purple-700 border border-purple-200">
            <CreditCard className="w-3 h-3 text-purple-600" /> KCC Limit Adjusted
          </span>
        )
      case 'CREDIT_MODEL_CALIBRATED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200">
            <Sliders className="w-3 h-3 text-amber-600" /> Credit Model Calibrated
          </span>
        )
      default:
        return (
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white/90 backdrop-blur-xl border border-emerald-100 rounded-2xl p-4 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900">Statutory Banking & Underwriting Audit Trail</h3>
            <p className="text-xs text-slate-500">
              Immutable audit ledger of penny-drop overrides, dual sign-off loan authorizations, KCC limits, and DPDP masking compliance sweeps
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <option value="PENNY_DROP_OVERRIDDEN">Penny-Drop Overrides</option>
            <option value="PRIMARY_ACCOUNT_DESIGNATED">Primary Account Changes</option>
            <option value="LOAN_UNDERWRITTEN_APPROVED">Loan Approvals (Dual-Sign)</option>
            <option value="LOAN_REJECTED">Loan Rejections</option>
            <option value="KCC_LIMIT_ADJUSTED">KCC Limit Adjustments</option>
            <option value="CREDIT_MODEL_CALIBRATED">Model Calibrations</option>
            <option value="DPDP_MASKING_AUDITED">DPDP Masking Audits</option>
          </select>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-gradient-to-r from-emerald-100/60 via-emerald-50/80 to-emerald-100/40 border-b border-emerald-200/80 text-emerald-950 uppercase tracking-wider font-bold text-[10px]">
                ...
                <th className="px-4 py-3.5">Target Entity / Facility</th>
            ...
            <tbody className="divide-y divide-emerald-100/60">
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="px-4 py-12 text-center text-slate-500">
                    No banking audit logs match the selected filter.
                  </td>
                </tr>
              ) : (

      ...
      {activeTab === 'json' && (
        <div className="space-y-3">
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-400 font-mono">Firestore Schema: `{itemType === 'account' ? 'bank_accounts' : itemType === 'loan' ? 'loan_applications' : 'kcc_records'}`</span>
            <button
              onClick={handleCopyJson}
              className="inline-flex items-center gap-1 text-slate-400 hover:text-emerald-400 text-xs px-2 py-1 rounded bg-slate-900 border border-slate-800"
            >
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/80 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-100 text-sm">{account.bankName}</span>
          </div>
          <div className="flex items-center gap-2">
            {account.isPrimary && (
        ...
        {isFailed && account.failureReason && (
          <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs flex items-start gap-2.5">
            ...
            <div>
              <p className="font-semibold">Penny-Drop Verification Failed:</p>
              <p className="mt-0.5 text-rose-200/90">{account.failureReason}</p>
            </div>
          </div>
        )}
        ...
        {account.verificationMethod === 'manualOverride' && (
          <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs">
            <p className="font-semibold">Manually Verified by Superadmin ({account.overrideVerifiedBy}):</p>
            <p className="mt-0.5 text-emerald-200/90">{account.overrideReason}</p>
          </div>
        )}
      </div>
...
function LoanOverview({ loan, onApprove, onReject, onOpenEmiCalculator }) {
  ...
  const isLargeFacility = loan.amount > 50000 // mandates dual sign-off per SOP-14 §6.3
  ...
  return (
    <>
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/80 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-xs text-slate-400 font-mono">Sanction Amount</span>
            <p className="text-2xl font-bold text-emerald-400 font-mono mt-0.5">{fmtINR(loan.amount)}</p>
          </div>
          <div className="text-right">
            ...
            <p className="text-[11px] text-slate-400 font-mono mt-1">Tenure: {loan.tenureMonths} Months</p>
          </div>
        </div>

        {isLargeFacility && (
          <div className="p-2.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs flex items-center justify-between">
            <div className="flex items-center gap-2">
              <ShieldAlert className="w-4 h-4 text-amber-400 shrink-0" />
              <span>SOP-14 §6.3 Mandate: Amount exceeds ₹50,000 (Requires Dual-Admin Sign-Off)</span>
            </div>
            {loan.dualSignOffAdmin ? (
              <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 rounded text-[10px] font-mono border border-emerald-500/40">
                Dual Signed: {loan.dualSignOffAdmin}
              </span>
            ) : (
              <span className="px-2 py-0.5 bg-amber-500/20 text-amber-300 rounded text-[10px] font-mono border border-amber-500/40">
        ...
        {isRejected && loan.rejectionReason && (
          <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs">
            ...
            <p className="mt-0.5 text-rose-200/90">{loan.rejectionReason}</p>
          </div>
        )}
      </div>
      ...
      <DrawerSection title="Underwriting Notes">
        <div className="p-3 rounded-lg border border-slate-800 bg-slate-900/50 text-xs text-slate-300 leading-relaxed font-sans">
  ...
  return (
    <>
      <div className="rounded-xl border border-slate-800 bg-slate-900/80 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-xs text-slate-400 font-mono">Sanctioned KCC Limit</span>
            <p className="text-2xl font-bold text-emerald-400 font-mono mt-0.5">{fmtINR(kcc.kccLimit)}</p>
          </div>
          <div className="text-right">
            <span
              className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
            ...
            <p className="text-[11px] text-slate-400 font-mono mt-1">Acreage: {kcc.sanctionedLandAcres} Acres</p>
          </div>
        </div>
  ...
  return (
    <>
      <div className="rounded-xl border border-slate-800 bg-slate-900/80 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-xs text-slate-400 font-mono">Installment Due</span>
            <p className="text-2xl font-bold text-emerald-400 font-mono mt-0.5">{fmtINR(milestone.emiAmount)}</p>
          </div>
          <div className="text-right">
            <span
              className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
            ...
            <p className="text-[11px] text-slate-400 font-mono mt-1">Due: {milestone.dueDate}</p>
          </div>
        </div>
      ...
      <div className="relative border-l border-slate-800 ml-3 space-y-6 py-2">
        {events.map((ev, i) => (
          <div key={i} className="relative pl-6">
            <span className="absolute -left-1.5 top-1 w-3 h-3 rounded-full bg-emerald-500 ring-4 ring-slate-950" />
            <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-3 text-xs space-y-1">
              <div className="flex justify-between items-center">
                <span className="font-mono font-bold text-emerald-400">{ev.action}</span>
                <span className="font-mono text-slate-500 text-[10px]">{ev.timestamp?.replace('T', ' ').slice(0, 19)}</span>
              </div>
              <p className="text-slate-300">{ev.details}</p>

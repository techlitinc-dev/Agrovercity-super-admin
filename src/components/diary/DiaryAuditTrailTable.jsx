...
export default function DiaryAuditTrailTable({
  auditLogs = [],
      ...
      case 'ENTRY_VERIFIED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
            <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Entry Verified (+15 🪙)
          </span>
        );
      case 'ENTRY_FLAGGED_FRAUD':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200">
            <AlertTriangle className="w-3 h-3 text-amber-600" /> Flagged Duplicate / Fraud
          </span>
        );
      case 'ENTRY_SOFT_DELETED_COINS_REVOKED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-200">
            <Trash2 className="w-3 h-3 text-rose-600" /> Soft-Deleted & Coins Revoked
          </span>
        );
      case 'PNL_STATEMENT_VERIFIED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-sky-50 text-sky-700 border border-sky-200">
            <FileText className="w-3 h-3 text-sky-600" /> P&L Statement Certified
          </span>
        );
      case 'BENCHMARK_CALIBRATED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200">
            <Sliders className="w-3 h-3 text-indigo-600" /> Cost Benchmark Calibrated
          </span>
        );
      default:
        return (
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white/90 backdrop-blur-xl border border-emerald-100 rounded-2xl p-4 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900">Farm Bookkeeping & P&L Oversight Audit Trail</h3>
            <p className="text-xs text-slate-500">Immutable audit record of diary verifications, AgriCoins clawbacks, fraud flags, and P&L bank certifications</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <option value="ENTRY_VERIFIED">Entry Verified</option>
            <option value="ENTRY_FLAGGED_FRAUD">Fraud Flags</option>
            <option value="ENTRY_SOFT_DELETED_COINS_REVOKED">Deletions & Reversals</option>
            <option value="PNL_STATEMENT_VERIFIED">P&L Certifications</option>
            <option value="BENCHMARK_CALIBRATED">Benchmark Calibrations</option>
          </select>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-gradient-to-r from-emerald-100/60 via-emerald-50/80 to-emerald-100/40 border-b border-emerald-200/80 text-emerald-950 uppercase tracking-wider font-bold text-[10px]">
                ...
                <th className="px-4 py-3.5">Target Entity / Voucher</th>
            ...
            <tbody className="divide-y divide-emerald-100/60">
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="px-4 py-12 text-center text-slate-500">
                    No bookkeeping audit logs match the selected filter.
                  </td>
                </tr>
              ) : (
